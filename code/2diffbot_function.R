...
diffbot = function(url, token){
  ...
  # get data
  response = tryCatch(getURL(request), error = function(e) "error")
  # json to R list
  data = NULL
  data = tryCatch(fromJSON(response), error = function(e) "error")
  # repeat once
  if(is.null(data) || length(data) < 1 || data[[1]] == "error") data = tryCatch(fromJSON(response), error = function(e) "error")
  if(is.null(data) || length(data) < 1 || !("text" %in% names(data))){
    return(data.table(
